...
public class JobInterviewActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        interviewDataList = readInterview();                                // ListView를 위한 정보를 받아옴(함수 사용)
    ...
    public List<InterviewData> readInterview() {                            // asset 폴더에서 정보를 읽기위한 함수
        List<InterviewData> list = new ArrayList<InterviewData>();                      // 읽은 정보를 담을 List(ListView에 사용됨)
        ...
        for(int i = 0; i < array.size(); i++) {                             // json 배열의 크기만큼 반복
            ...
            list.add(new InterviewData(jObject.get("job").getAsString(), jObject.get("url").getAsString()));
        }
